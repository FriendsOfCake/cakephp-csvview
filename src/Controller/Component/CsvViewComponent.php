<?php
namespace CsvView\Controller\Component;
...
class CsvViewComponent extends Component
{
     ...
     */
    public function prepareExtractFromFindResults($data, $excludePaths = [])
    {
        $extract = [];
     ...
     */
    public function prepareHeaderFromExtract($extract, $customHeaders = [], $options = [])
    {
        $defaults = ['includeClassname' => true, 'humanReadable' => true];
        $options += $defaults;
     ...
     * @return void
     */
    public function quickExport($data, $excludePaths = [], $customHeaders = [], $options = [])
    {
        // Maintain backwards compatiblity if someone passes a boolean includeHeader instead of an options array
        if (!is_array($options)) {
            $options = ['includeHeader' => (bool)$options];
        }

        $defaults = ['includeHeader' => true, 'includeClassname' => true, 'humanReadable' => true];
        $options += $defaults;
        ...
        $controller->set(compact('data', '_serialize', '_header', '_extract'));
    }
}
